class Graph:
    ...
    def shortest_cycle_path(self, n, l):
        ...
        def dfs(current, length, path):
            ...
            elif length < n - l:  # Pruning
                for vertex, cost in self.incident_list[current]:
                    if vertex not in visited:
                        new_length = length + cost
                        if new_length not in cache or len(path) > len(cache[new_length]):
                            cache[new_length] = path.copy()  # Store result in cache
                            dfs(vertex, new_length, path)
    ...
    def calculate_errors(self, n, l, best_path):
        ...
        # print("obliczanie neg:", n-l+1, "-", len(best_path), "=", negative)
        positive = len(self.incident_list.keys()) - len(best_path)
        # print("obliczanie pos:", len(self.incident_list.keys()), "-", len(best_path),"=",positive)
        # print(best_path)
        errors = positive - negative
        '''if errors > 0:
            # print("Uzyskane błędy pozytywne:", errors)
            return errors, 0
        elif errors < 0:
            # print("Uzyskane błędy negatywne:", -errors)
            return 0, -errors
        else:
            # print("Brak błędów")
            return 0, 0'''
